...
try {
  for (const dpr of [1, 2]) {
    ...
    // Repeated selection verifies CSS-space pointers still map correctly after
    // the backing store is denser and after the selector tree has re-rendered.
    analytics.length = 0;
    ...
    report.push({ dpr, metrics, interactionChecks: ['drop', 'charged', 'basic', 'charged-again', 'collection', 'library'], runtimeErrors });
    await context.close();
  }
} finally {
  await browser.close();
