...
const OUT = 'audit-secret-presence-r3';
...
const runHidden = async (index) => {
  ...
  const secret = await page.evaluate(() => {
    const scene = window.__auditGame.scene.getScene('OpeningScene');
    const sparkles = scene.secretPremiumTargets
      .filter((target) => target?.active && target.type === 'Arc' && Number(target.radius ?? 999) < 4)
      .map((target) => ({
        x: Math.round(Number(target.x) * 10) / 10,
        y: Math.round(Number(target.y) * 10) / 10,
        r: Math.round(Number(target.radius) * 10) / 10,
      }));
    const audio = window.__auditAudio;
    return {
      index: scene.resultCarouselIndex,
      sparkles,
      signature: sparkles.map(({ x, y, r }) => `${x}:${y}:${r}`).join('|'),
  ...
  if (secret.sparkles.length !== 10) throw new Error(`expected 10 persistent Secret sparkles, got ${secret.sparkles.length}`);
  ...
  const standard = await page.evaluate(() => {
    const scene = window.__auditGame.scene.getScene('OpeningScene');
    const audio = window.__auditAudio;
    return { index: scene.resultCarouselIndex, desired: audio?.desiredResultAmbience ?? null, active: audio?.activeResultAmbience ?? null, sources: audio?.persistentRaritySources?.length ?? null };
  });
  if (standard.index !== 0 || standard.desired === 'secret') throw new Error(`standard carousel state mismatch: ${JSON.stringify(standard)}`);
  ...
  if (restored.index !== 1 || restored.desired !== 'secret') throw new Error(`Secret restore mismatch: ${JSON.stringify(restored)}`);
  ...
  const idle = await page.evaluate(() => {
    const scene = window.__auditGame.scene.getScene('OpeningScene');
    const audio = window.__auditAudio;
    return { secretTargets: scene.secretPremiumTargets.length, standardTargets: scene.standardPresenceTargets.length, desired: audio?.desiredResultAmbience ?? null, active: audio?.activeResultAmbience ?? null, sources: audio?.persistentRaritySources?.length ?? null };
  });
  if (idle.secretTargets !== 0 || idle.standardTargets !== 0 || idle.desired !== null || idle.sources !== 0) throw new Error(`idle teardown mismatch: ${JSON.stringify(idle)}`);
...
if (new Set(signatures).size !== 3) throw new Error(`Secret sparkle composition repeated: ${JSON.stringify(signatures)}`);
