...
const OUT = 'audit-secret-presence-r2';
...
const runHidden = async (index) => {
  ...
  const secret = await page.evaluate(() => {
    ...
    const sparkles = scene.secretPremiumTargets
      .filter((target) => target?.active && target.type === 'Arc' && Number(target.displayWidth ?? 999) < 10)
      .map((target) => ({
    ...
    return {
      ...
      sparkles,
      signature: sparkles.map(({ x, y, w }) => `${x}:${y}:${w}`).join('|'),
