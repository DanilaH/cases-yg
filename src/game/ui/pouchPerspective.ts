...
const FRAGMENT_SHADER = [
  ...
  '        float pouchMaterial = smoothstep(0.084, 0.09, sheenStrength);',
  '        float baseSheen = 1.0 - smoothstep(0.055, 0.18, abs(sheenCoord - sheenCenter));',
  '        float pouchSheen = 1.0 - smoothstep(0.07, 0.24, abs(sheenCoord - sheenCenter));',
  '        float sheen = mix(baseSheen, pouchSheen, pouchMaterial);',
  ...
  '        float effectiveSheenStrength = sheenStrength * mix(1.0, 1.35, pouchMaterial);',
  '        float effectiveRimStrength = rimStrength * mix(1.0, 1.18, pouchMaterial);',
  '        float sheenMotion = mix(0.08 + 0.92 * motion, 0.14 + 1.06 * motion, pouchMaterial);',
  '        float rimBias = mix(0.25 + 0.75 * nearBias, 0.30 + 0.82 * nearBias, pouchMaterial);',
  '        float sheenAmount = effectiveSheenStrength * sheen * sheenMotion * alphaMask;',
  '        float rimAmount = effectiveRimStrength * rimMask * rimBias * alphaMask;',
  ...
  '        {',
  '            vec2 centered = uv - vec2(0.5, 0.69);',
  ...
  '            float width = smoothstep(0.16, 1.0, crackProgress) * crackStrength * 0.10;',
  ...
  '            cover *= smoothstep(0.34, 0.40, uv.y) * (1.0 - smoothstep(0.87, 0.96, uv.y));',
  '            sampled.rgb += crackTint * (0.48 * hot + 0.35 * ember) * crackStrength * cover * smoothstep(0.12, 0.65, sampled.a);',
  '            sampled.rgb += vec3(0.12) * ember * crackStrength * cover * smoothstep(0.12, 0.65, sampled.a);',
