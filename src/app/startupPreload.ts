...
export const computeStartupFakeProgress = (elapsedMs: number, cap = DEFAULT_OPTIONS.preCompleteCap): number => {
...
export class StartupPreloadController {
  ...
  private copy: StartupPreloadCopy = DEFAULT_COPY;
  ...
  public complete(): void {
    ...
    // A timeout is diagnostic, not a permanent lockout. If an unusually slow
    // platform bootstrap eventually reaches the real presentable-ready signal,
    // recover cleanly instead of leaving a false terminal overlay over the game.
  ...
  private tickProgress(): void {
    ...
    const elapsedMs = this.scheduler.now() - this.startedAt;
    this.progress = computeStartupFakeProgress(elapsedMs, this.options.preCompleteCap);
    this.emit();
  }
  ...
  private emit(): void {
    const snapshot: StartupPreloadSnapshot = this.failureMessage === undefined
      ? { phase: this.phase, progress: this.progress, copy: this.copy }
      : { phase: this.phase, progress: this.progress, copy: this.copy, failureMessage: this.failureMessage };
    this.view.render(snapshot);
  }
}

export const createStartupPreloadDomView = (root: Document = document): StartupPreloadView => {
  const overlay = root.querySelector<HTMLElement>('#startup-preload');
  const heading = root.querySelector<HTMLElement>('#startup-preload-heading');
  const status = root.querySelector<HTMLElement>('#startup-preload-status');
  const progressBar = root.querySelector<HTMLElement>('#startup-preload-progress-bar');
  const progressFill = root.querySelector<HTMLElement>('#startup-preload-progress-fill');
  const progressPercent = root.querySelector<HTMLElement>('#startup-preload-progress-percent');
