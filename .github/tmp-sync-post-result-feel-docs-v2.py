...
replace_once(
    "docs/OPENING_FEEL_CORRECTION_SCOPE.md",
    block("---", "", "# 3. Input and pacing — highest priority"),
    block(
        "---",
        "",
...
replace_once(
    ...
    block("- CHIPS count-up / local HUD reaction / synthesized `chip-clack`;", "- synth-only `pouch-grab` feedback on star grab;"),
)
replace_once(
    "docs/PRODUCT.md",
...
# GAMEPLAY_SYSTEMS
replace_once("docs/GAMEPLAY_SYSTEMS.md", "star grab response\n→ drag tension", "star grab response + `pouch-grab` material cue\n→ drag tension")
replace_once(
...
replace_once("docs/OPEN_QUESTIONS.md", "Dedicated `chips-collect` SFX is integrated from hands-on evidence.", "The current CHIPS banking cue is synthesized `chip-clack`; star grab also has a synth-only `pouch-grab` cue from later hands-on evidence.")
...
replace_once("docs/ASSET_MANIFEST.md", "- staged reward tray near hero;", "- staged reward tray near hero with single-entry continuity into the later result panel;")
replace_once("docs/ASSET_MANIFEST.md", "- Common/Rare/Epic/Legendary escalating shimmer;", block("- Common/Rare/Epic/Legendary escalating shimmer + larger rarity capsule/tinted result border;", "- semantic reward-row colors including rarity-aware recycle copy;"))
