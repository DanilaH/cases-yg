...
// A. Fresh compact account: disabled Charged, partial tear cancellation, mute persistence, empty Collection.
...
// Mute should persist separately from gameplay save.
await tap(fresh, 710, 52);
await fresh.waitForTimeout(350);
await shot(fresh, '04-muted');
const settingsAfterMute = await fresh.evaluate((key) => {
  const raw = localStorage.getItem(key);
  return raw ? JSON.parse(raw) : null;
}, SETTINGS_KEY);
assertions.push({ name: 'mute-persists', pass: settingsAfterMute?.muted === true, observed: settingsAfterMute });
await fresh.reload({ waitUntil: 'domcontentloaded' });
await waitLandscape(fresh);
await hideDebug(fresh);
await shot(fresh, '05-muted-after-reload');

// Empty collection should be a coherent real state, not a blank/error surface.
await tap(fresh, 705, 664);
await fresh.waitForTimeout(650);
await shot(fresh, '06-empty-collection');
await tap(fresh, 76, 664);
await fresh.waitForTimeout(600);
await shot(fresh, '07-return-from-empty-collection');
...
assertions.push({ name: 'rapid-result-taps-are-idempotent', pass: save?.totalOpens === 1 && save?.pendingReveal === null, observed: save });
...
assertions.push({ name: 'result-survives-orientation-block', pass: save?.totalOpens === 1 && save?.pendingReveal === null, observed: save });
...
await waitCommitted(recovery, 1);
await recovery.waitForTimeout(850);
await shot(recovery, '31-recovery-result-settled');
save = await readSave(recovery);
assertions.push({ name: 'pending-recovery-commits-once', pass: save?.totalOpens === 1 && save?.pendingReveal === null, observed: save });
