...
const outDir = '/tmp/opening-feel-v2-r1';
const errors = [];
...
const customPendingSave = ({
  ...
  const beforeSignal = 0;
  const afterSignal = beforeSignal + signalGain;
  ...
  const nextDiscovered = recycle > 0 ? discoveredStandard : [...discoveredStandard, collectibleId];
  const pending = {
    id: `audit-${Date.now()}-${base}-${cacheBonus}-${recycle}-${signalGain}`,
    baseTotalOpens: 6,
    pouchType,
...
const acceptResult = async (page, x, y) => {
  // First deliberate tap finishes the read beat; second accepts.
...
const waitForIdleAfterBank = async (page, expectedChips) => {
  ...
  }, { key: SAVE_KEY, chips: expectedChips }, { timeout: 15_000 });
  // The save is committed before cosmetic banking, so also wait for the visual sequence.
  await page.waitForTimeout(1250);
};

const makeContext = (width, locale, tag) => browser.newContext({
  viewport: { width, height: 720 },
  locale,
  recordVideo: { dir: outDir, size: { width, height: 720 } },
});

// Compact RU: HUD density, wallet width, selector states, 4-row result, exact visible banking.
const compactContext = await makeContext(900, 'ru-RU', 'compact');
...
await setSaveAndReload(compact, baseSave({ chips: 999999, signal: 4, discoveredStandard: [
  'camera-common','camera-rare','camera-epic','flip-phone-common','flip-phone-rare','flip-phone-epic',
] }));
await shot(compact, '00-ru-900-six-digit-lock-hud');

await setSaveAndReload(compact, baseSave({ chips: 20 }));
await shot(compact, '01-ru-900-unavailable-charged');
...
await shot(compact, '02-ru-900-unavailable-feedback');
...
await shot(compact, '03-ru-900-four-row-result');
await acceptResult(compact, 450, 568);
for (let i = 0; i < 9; i += 1) {
  await compact.waitForTimeout(85);
  await shot(compact, `04-ru-900-bank-${i}`);
}
await waitForIdleAfterBank(compact, 64);
await shot(compact, '05-ru-900-after-bank-idle');
await compactContext.close();

// Mid-width EN: 1/2/3-row tray geometry and Charged spend visibility.
const midContext = await makeContext(1024, 'en-US', 'mid');
...
for (const [rows, save] of [
  ...
  await shot(mid, `06-en-1024-${rows}-row-result`);
}

await setSaveAndReload(mid, baseSave({ chips: 100 }));
await stageScenario(mid, 'Force Legendary (Charged)');
// Capture early recovered Charged spend/reveal path after reload.
await mid.waitForTimeout(160);
await shot(mid, '07-en-1024-charged-recovery-early');
await waitForCommittedResult(mid);
await shot(mid, '08-en-1024-charged-result');
await midContext.close();

// Wide EN: normal duplicate/Signal banking, Hidden Pocket, selector regression.
const wideContext = await makeContext(1280, 'en-US', 'wide');
...
await shot(wide, '09-en-1280-charged-selected');
await humanTap(wide, 160, 235);
await wide.waitForTimeout(700);
await shot(wide, '10-en-1280-basic-reselected');
...
await shot(wide, '11-en-1280-signal-lock-result');
await acceptResult(wide, 640, 568);
for (let i = 0; i < 8; i += 1) {
  await wide.waitForTimeout(80);
  await shot(wide, `12-en-1280-signal-bank-${i}`);
}
await shot(wide, '13-en-1280-after-signal-bank');

await setSaveAndReload(wide, baseSave({ chips: 80 }));
await stageScenario(wide, 'Force Hidden Pocket');
await waitForCommittedResult(wide);
await shot(wide, '14-en-1280-hidden-pocket-result');
