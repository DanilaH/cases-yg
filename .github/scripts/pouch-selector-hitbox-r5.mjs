...
const outDir = '/tmp/pouch-selector-hitbox-r5';
...
const waitReady = async page => { await page.waitForSelector('#game canvas', { state: 'visible', timeout: 20000 }); await page.waitForFunction(() => document.querySelector('#orientation-gate')?.getAttribute('data-visible') !== 'true'); await page.waitForTimeout(500); await page.evaluate(() => { const p = document.querySelector('.mpt-debug-panel'); if (p) p.style.display = 'none'; }); };
...
const tap = async (page, x, y) => { await page.mouse.move(x, y); await page.mouse.down(); await page.waitForTimeout(80); await page.mouse.up(); };
const settleSelector = page => page.waitForTimeout(700);
const tear = async (page, width) => { const cx = width / 2; await page.mouse.move(cx - 175, 174); await page.mouse.down(); await page.mouse.move(cx + 200, 174, { steps: 24 }); await page.mouse.up(); };
const pending = async page => { for (let i = 0; i < 120; i++) { const s = await page.evaluate(k => { const r = localStorage.getItem(k); if (!r) return null; const v = JSON.parse(r); return { chips: v.chips, pendingType: v.pendingReveal?.pouchType ?? null, totalOpens: v.totalOpens }; }, SAVE_KEY); if (s?.pendingType) return s; await page.waitForTimeout(35); } return null; };
...
for (const width of [900, 1280]) {
  ...
  for (const [name, x, y] of points(r.charged)) {
    await reset(page, 100); await tap(page, x, y); await settleSelector(page);
    if (name === 'top-left' || name === 'bottom-right') await page.screenshot({ path: `${outDir}/${width}-charged-${name}.png` });
  ...
  for (const [name, x, y] of points(r.basic)) {
    await reset(page, 100); await tap(page, r.charged.left + 44, r.charged.top + 12); await settleSelector(page); await tap(page, x, y); await settleSelector(page);
  ...
  await reset(page, 20); await tap(page, r.charged.left + r.charged.width - 8, r.charged.top + r.charged.height - 8); await page.waitForTimeout(120); await page.screenshot({ path: `${outDir}/${width}-unavailable.png` });
