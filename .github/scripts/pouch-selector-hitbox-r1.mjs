...
const PRODUCT_HEAD = 'f0b6a37f7000fa48020b57d38ae2b19ce4c12eb9';
const SAVE_KEY = 'mystery-pocket-tech.save';
const outDir = '/tmp/pouch-selector-hitbox-r1';
const errors = [];
...
const auditViewport = async (viewportWidth) => {
  ...
  // Every visible Charged point must select Charged and drive a Charged reveal.
  ...
  // Start from Charged, then every visible Basic point must switch back to Basic.
  for (const [name, x, y] of pointsFor(rects.basic)) {
    await setSaveAndReload(page, 100);
    // Known interior point of Charged after the fix.
  ...
  // Unavailable Charged must remain non-mutating even at the old failing lower-right region.
